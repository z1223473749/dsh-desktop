...
- This project pins both the published DSH `0.1.7-rc.1` family and the corresponding official `deepseek-harness/` release source. Product builds consume the checked-in official-profile runtime tarballs recorded in `upstream.json`, rather than linking the source checkout.
- DSH `0.1.7-rc.1` migrates supported historical sessions to V4 while preserving the original logs. Sessions written after upgrading cannot be read by the previous `0.1.5-rc.2` runtime.
